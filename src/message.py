  ...
  <div style="background:#f9f9f9;padding:20px;border:1px solid #ddd;border-radius:0 0 8px 8px;text-align:center;">
    ...
    for g in games:
        lines += [
            ...
            f"  Pick : {g['favorite'] or 'Even / No line'}",
            divider,
        ]
    lines.append("")
    lines.append(f"Jersey Minders Win Rate: {win_rate}")
    plain = "\n".join(lines)

    # --- HTML ---
    html = _build_html(today, games, win_rate)
    return plain, html


def build_discord_payload(games: list[dict], subject: str, win_rate: str = "No results yet") -> dict:
...
def _build_html(today: str, games: list[dict], win_rate: str) -> str:
    rows = ""
...
<body style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;padding:20px;background:#f0f0f0;">
  ...
  <div style="background:#f9f9f9;padding:14px;border-radius:0 0 10px 10px;text-align:center;border:1px solid #ddd;border-top:none;">
    <span style="color:#555;font-size:13px;">Jersey Minders Win Rate: </span>
    <strong style="color:#c8102e;font-size:15px;">{win_rate}</strong>
  </div>
